"""
Generate Accuracy-vs-Epoch plots for MLP_Model_Monk runs.
...
class AccuracyCurveCallback(pl.Callback):
    def __init__(
        self,
        *,
    ...
    def on_train_epoch_end(self, trainer: pl.Trainer, pl_module: pl.LightningModule) -> None:
        ...
        test_mse, test_acc = eval_loader_metrics(pl_module, self.test_loader, device)
        if was_training:
            pl_module.train()
...
def _discover_targets(mse_root: Path) -> list[Target]:
    ...
    for task_dir in sorted(mse_root.glob("monk*")):
        ...
        for mse_file in sorted(task_dir.glob("*_mse_curve.png")):
            ...
            out_name = mse_file.name.replace("mse_curve", "accuracy_curve")
            targets.append(Target(task_id=task_id, want_reg=want_reg, out_path=out_dir / out_name))
    return targets
...
def _run_target(
    ...
    *,
    ...
    callback = AccuracyCurveCallback(
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate accuracy curves for MLP_Model_Monk.")
    parser.add_argument(
        "--mse-root",
        type=Path,
