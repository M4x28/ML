...
"""Dataset loading and splitting for ML-CUP."""
...
def load_cup_test(path: str | Path) -> tuple[np.ndarray, np.ndarray]:
    """Load test CSV and return (ids, X) arrays (no targets)."""
    df = _read_cup_csv(path)
    if df.shape[1] < 2:
...
def split_train_val_test(
    ...
    *,
    ...
    """Return train/val/test splits in a dict of numpy arrays."""
    if val_ratio < 0 or test_ratio < 0 or (val_ratio + test_ratio) >= 1:
        raise ValueError("val_ratio + test_ratio must be in [0, 1).")

    # Initialize outputs for the case of no holdout split.
    X_train, y_train, ids_train = X, y, ids
    X_val, y_val, ids_val = None, None, None
    X_test, y_test, ids_test = None, None, None

    # First create a train/holdout split, then split holdout into val/test.
